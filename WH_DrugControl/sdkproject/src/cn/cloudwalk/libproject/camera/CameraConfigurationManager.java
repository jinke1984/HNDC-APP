...
final class CameraConfigurationManager {
	...
	public void setDesiredCameraParameters(Camera camera, int caremaId, boolean isModeAuto) {
		...
		if (isModeAuto) {
			...
			String md_old = parameters.getFocusMode();//TODO
			List<String> focusModes = parameters.getSupportedFocusModes();
			for (String it : focusModes) {
				LogUtils.LOGE("mode", "" + it);
				if (it.equals(Camera.Parameters.FOCUS_MODE_INFINITY)) {
					parameters.setFocusMode(it);
					LogUtils.LOGE("mode", "set = " + it);
				}
			}
			String md_new = parameters.getFocusMode();
		}
	...
	public int setCameraParametersForPreviewCallBack(Camera camera, int caremaId, int previewW, int previewH) {
		...
		int degree = getDisplayOrientation(caremaId);
		//camera.setDisplayOrientation(getDisplayOrientation(caremaId));
...
//
//		if (bigEnough.size() > 0) {
//			return Collections.min(bigEnough, new CameraTextureView.CompareSizesByArea());
//		} else {
//			return choices[0];
//		}
//	}
