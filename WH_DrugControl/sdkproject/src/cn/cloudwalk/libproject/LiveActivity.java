...
public class LiveActivity extends TemplatedActivity implements LivessCallBack, FaceInfoCallback {
    ...
    final static int NEXT_STEP = 101, UPDATE_STEP_PROCRESS = 106, SET_RESULT = 122,
            ...
            PLAYMAIN_END = 125,
            DETECT_READY = 126,
            PREPARE_RESET_INFO = 127;//重置准备阶段提示信息
    boolean isStop;// 界面遮盖
    ...
    static final int DURATION_PREPARE_INFO = 3000;//准备阶段算法返回提示,UI处理时间间隔
    ...
    private void initcloudwalkSDK() {
        cloudwalkSDK = cloudwalkSDK.getInstance();
            ...
            public void run() {
                ...
                if (currentInfoTime - mLastPrepareInfoTime >DURATION_PREPARE_INFO){
                    ...
                    if (currentStep ==0 && viewList!=null && viewList.get(0)!=null) {
                        ...
                        mMainHandler.sendEmptyMessageDelayed(PREPARE_RESET_INFO,DURATION_PREPARE_INFO/2);//准备阶段1s后更新提示
                    }
                }
            }
        });
    ...
    public void detectReady() {
        ...
        mMainHandler.removeMessages(PREPARE_RESET_INFO);//情况重置准备阶段提示
    ...
    public void OnActionNotStandard(int notStandardType) {
        ...
        if (currentStep != 0 && !isLivePass) {
            if (cloudwalkSDK != null) {
                cloudwalkSDK.cwStopLivess();
                cloudwalkSDK.cwResetLivenessTarget();//重置活体检测器
            }
                ...
                case NEXT_STEP:
                    if (activity.totalStep == activity.currentStep) {// 人脸比对页面
                        ...
                        if (activity.cloudwalkSDK != null) {
                            activity.cloudwalkSDK.cwResetLivenessTarget();//重置活体检测器
                        }
                        if (Bulider.isServerLive) {
...
//                            activity.clearBestFace();
                ...
                case DETECT_READY:
                    ...
                    break;
                case PREPARE_RESET_INFO://重置准备阶段提示信息
                    removeMessages(PREPARE_RESET_INFO);
                    if (activity.currentStep ==0 && activity.viewList!=null && activity.viewList.get(0)!=null) {
                        TextView tv_Info = (TextView)activity.viewList.get(0).findViewById(R.id.cloudwalk_face_info_txt);
                        if (tv_Info!=null) {
                            tv_Info.setText(R.string.cloudwalk_tip_not_center);
                            break;
                        }
        ...
        public void run() {
            ...
            if (djsCount >= 0) {
                ...
                if (act.cloudwalkSDK != null) {
                    ...
                    act.cloudwalkSDK.cwResetLivenessTarget();//重置活体SDK
                }
