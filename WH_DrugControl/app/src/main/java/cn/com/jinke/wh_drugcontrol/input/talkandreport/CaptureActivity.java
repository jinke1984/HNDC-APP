 ...
 * @author Sean Owen
 */
public final class CaptureActivity extends ProjectBaseUI implements SurfaceHolder.Callback, MsgKey{
	...
	public static final int DECODING = 7;
	...
	private boolean isHomePage = false;  //true代表在首页扫描，false代表在其他页面扫描

	int [] MSG = new int[]{MSG_SCAN_PERSION_INFO};
	...
	private void initData(){
		...
		if(intent != null){
			isHomePage = intent.getBooleanExtra(B_CODE, isHomePage);
		}
	}

	@Override
	protected boolean handleMessage(Message msg) {
		if(msg.what == MSG_SCAN_PERSION_INFO){
			dismissLoading();
			DruguserslistEntity druguserslistEntity = (DruguserslistEntity) msg.obj;
			if(druguserslistEntity != null){
				UserCard userCard = MasterManager.getInstance().getUserCard();
				final String userId = userCard.getUserId();
				String controlId = druguserslistEntity.getControlId();
				if(!userId.equals(controlId)){
					showToast(getString(R.string.zmgkrybsy));
				}else{
					int status = druguserslistEntity.getCurrentStateInt();
					switch (status) {
						case 1:
						case 3:
							int type = druguserslistEntity.getType();
							switch (type) {
								case 2:
									//ReportDetailUI.startActivity(CaptureActivity.this, dle, id, area, type, true);
//									ReportSaveUI.startActivity(CaptureActivity.this, dle, id, area, type, true);
									MessageProxy.sendMessage(MSG_SCAN_GET_QRCODE,1,druguserslistEntity);
									break;
								case 4:
									MessageProxy.sendMessage(MSG_SCAN_GET_QRCODE,1,druguserslistEntity);
									//TalkDetailUI.startActivity(CaptureActivity.this, dle, id, area, type, true);
//									TalkSaveUI.startActivity(CaptureActivity.this, dle, id, area, type, true);
									break;
								default:
									showToast(getString(R.string.qmqbzc));
									break;
							}
							break;
						case 2:
							showToast(getString(R.string.ccutse));
							break;
						case 4:
							showToast(getString(R.string.ccutss));
							break;
						case 9:
							showToast(getString(R.string.ccutsj));
							break;
						default:
							break;
					}
	...
	public void handleDecode(Result rawResult, Bitmap barcode) {
		...
		if (barcode == null) {
			...
			if (resultString.equals("")) {
				Toast.makeText(CaptureActivity.this, getString(R.string.qsmsb), Toast.LENGTH_SHORT).show();
			} else {
				...
				if(isHomePage){
					...
					if(array.length != Constans.QRCODE_MAX){
						showToast(getString(R.string.qsmsb));
						CaptureActivity.this.finish();
					}else{
						String drugIdCard = array[0];
						String type = array[1];
						if(type.equals("2") || type.equals("4")){
							//getQRCode(drugIdCard, type);
						}else{
							showToast(getString(R.string.qmqbzc));
							CaptureActivity.this.finish();
						}
...
//	private void getQRCode(final String idCard, final String aType){
//		showLoading();
//		RequestParams requestParams = new RequestParams(API_DRUGINFO);
//		requestParams.addParameter("idAndIdCard",idCard);
//		requestParams.addParameter("is_qrcode","1");
//		x.http().post(requestParams,new CallbackWrapper<DruguserslistEntity>(MSG_SCAN_PERSION_INFO,2){
//			@Override
//			public void onSuccess(int state, String msg, DruguserslistEntity object, int total) {
