...
import cn.com.jinke.wh_drugcontrol.receiver.HomeKeyReceiver;
import cn.com.jinke.wh_drugcontrol.receiver.ScreenReceiver;
...
import cn.com.jinke.wh_drugcontrol.utils.NetworkHelper;
...
import cn.com.jinke.wh_drugcontrol.utils.VersionHelper;

/**
 * Created by jinke on 16/8/1.
 */
public class ProjectBaseUI extends Activity implements JsonConstans, MsgKey, CodeConstants, ShareConstant, IDocumentType {
    ...
    private final HomeKeyReceiver mHomeKeyReceiver = new HomeKeyReceiver();
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        registerMessages(FINISH_ALL_ACTIVITY,ACCESS_NET_FAILED);
        ...
        mHomeKeyReceiver.setOnHomeKeyListener(new HomeKeyReceiver.OnHomeKeyListener() {
            @Override
        ...
        mScreenReceiver.setOnScreenListener(new ScreenReceiver.OnScreenListener() {
            @Override
            public void onScreenOn() {
                final boolean useGesture = ShareUtils.getInstance().get(ShareUtils.ESHARE.SYS, SP_KEY_USE_GESTURE, ShareUtils.ETYPE.BOOL);
                boolean reLogin = ShareUtils.getInstance().get(ShareUtils.ESHARE.SYS, SP_KEY_RELOGIN, ShareUtils.ETYPE.BOOL);

                if (useGesture && !Utils.isApplicationToBackground(ProjectBaseUI.this) && !reLogin) {
                    ARouter.getInstance().build(RouteUtils.R_GESTURE_PWD_UI).navigation();
                }
            }
    ...
    protected void onStart() {
        super.onStart();
        mHomeKeyReceiver.register(this);
        final boolean useGesture = ShareUtils.getInstance().get(ShareUtils.ESHARE.SYS, SP_KEY_USE_GESTURE, ShareUtils.ETYPE.BOOL);
        boolean reLogin = ShareUtils.getInstance().get(ShareUtils.ESHARE.SYS, SP_KEY_RELOGIN, ShareUtils.ETYPE.BOOL);

        if (isHomeKeyPressed) {
            isHomeKeyPressed = false;
            if (useGesture && !reLogin) {
                ARouter.getInstance().build(RouteUtils.R_GESTURE_PWD_UI).navigation();
            }
        }
    }
