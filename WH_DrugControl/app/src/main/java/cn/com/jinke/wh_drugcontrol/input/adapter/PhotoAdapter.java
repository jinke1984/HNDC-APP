...
/**
 * Created by jinke on 2017/9/6.
 */

public class PhotoAdapter extends ProjectBaseAdapter<String> {
    ...
    public View getViewEx(int position, View v, String s) {
        Holder holder = null;
        if(v == null){
            v = inflater.inflate(R.layout.item_talk_photo, null, false);
            holder = new Holder(v);
            v.setTag(holder);
        }else{
    ...
    private class Holder {
        ...
        public void setItem(String path){
            if(path != null && !TextUtils.isEmpty(path) && !path.equals("null")){
